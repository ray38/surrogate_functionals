# -*- coding: utf-8 -*-
"""
Created on Thu Sep 14 15:22:47 2017
...
#                    print stencil_list[index]
    ...
    #                result.append(carve_out_matrix(temp_data).flatten().tolist())
                if r == 0.2:
                    result.append(np.around(carve_out_matrix(temp_data),9).flatten().tolist())
...
#        derivative_grp.create_dataset('derivative_3',data=carve_out_matrix(temp_third_deri))
        result.append(np.around(carve_out_matrix(temp_first_deri),9).flatten().tolist())
        result.append(np.around(carve_out_matrix(temp_sec_deri),9).flatten().tolist())
        print data.keys()
...
def process(molecule, functional,i,j,k,h,N,r_list,stencil_list,pad_list):
    result_filename = "{}_{}_{}_{}_{}_all_descriptors.hdf5".format(molecule,functional,i,j,k)
    result_csv_filename = "{}_{}_{}_{}_{}_all_descriptors.csv".format(molecule,functional,i,j,k)
    if os.path.isfile(result_filename) == False:
        print 'start {} {} {}'.format(i,j,k)
        temp1 = process_normal_descriptors(molecule, functional,i,j,k)
        temp2 = process_range_descriptor(molecule, functional,i,j,k,h,N,r_list,stencil_list,pad_list)
        result = np.stack((temp1+temp2),axis=1).tolist()
...
#            writer.writerow(['x','y','z','rho','gamma','tau','Vxc','epxc','ad_0-01','ad_0-02','ad_0-03','ad_0-04','ad_0-05','ad_0-06','ad_0-08','ad_0-1','ad_0-15','ad_0-2','ad_0-3','ad_0-4','ad_0-5','deriv_1','deriv_2'])
            writer.writerow(['x','y','z','rho','gamma','epxc','ad0p2','deriv1','deriv2'])
...
#    pool.join()
    ...
    for i,j,k in paramlist:
        process(molecule, functional,i,j,k,h,N,r_list,stencil_list,pad_list)
#    pool.close()
#    pool.join()
