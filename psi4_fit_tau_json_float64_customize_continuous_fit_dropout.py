...
def fit_with_KerasNN(X, y, loss, tol, slowdown_factor, early_stop_trials):
    ...
    while keep_going:
        ...
        if new_loss < best_loss:
            ...
            best_model = model
            if loss == "sae":
                log(log_filename, "\n updated best: "+ str(new_loss) + " \t epochs since last update: " + str(count_epochs) + " \t loss: " + loss + " \t projected error: " + str(((new_loss)*0.02*0.02*0.02*27.2114)*125/3)  )
            else:
...
def fit_model(dens, X_train, residual, loss, tol, slowdown_factor, early_stop_trials):
    ...
    NN_model,loss_result = fit_with_KerasNN(X_train, residual, loss, tol, slowdown_factor, early_stop_trials)
...
if __name__ == "__main__":
    ...
    model_save_dir = working_dir + "/" + "NN_{}_{}_{}_Dropout_test".format(setup["NN_setup"]["number_neuron_per_layer"], setup["NN_setup"]["number_layers"], setup["NN_setup"]["activation"])
    ...
    for fit_setup in fit_setup['fit_setup']:
        ...
        tol = fit_setup['tol']
        fit_model(dens, X_train, y, loss, tol, slowdown_factor, early_stop_trials*10)
