...
def predict_each_block(setup,dens,X,y):
    ...
    standard_scaler = setup["standard_scaler"]
    polynomial_order = setup["polynomial_order"]
    ...
    raw_predict_y = predict_poly_model(X_standard, poly_model,polynomial_order)
...
def load_data_each_block(molecule,functional,i,j,k, dataset_setup, data_dir_full):
    ...
    try:
        ...
        if len(temp_list) > 0:
            for r_list_count in temp_list:
                ...
                temp_data = np.asarray(data[group_name][dataset_name])
                result_list.append(transform_data(temp_data, dataset_setup['MCSH_0_1_transform']))
    except:
        pass
    ...
    try:
        temp_list = dataset_setup["MCSH_2_2_r_list"]
        if len(temp_list) > 0:
            for r_list_count in temp_list:
                dataset_name = 'MCSH_2_2_{}'.format(str(r_list_count).replace('.','-'))
                temp_data = np.asarray(data[group_name][dataset_name])
                result_list.append(transform_data(temp_data, dataset_setup['MCSH_2_2_transform']))
    except:
        pass
    ...
    try:
        temp_list = dataset_setup["MCSH_4_1_r_list"]
        if len(temp_list) > 0:
            for r_list_count in temp_list:
                dataset_name = 'MCSH_4_1_{}'.format(str(r_list_count).replace('.','-'))
                temp_data = np.asarray(data[group_name][dataset_name])
                result_list.append(transform_data(temp_data, dataset_setup['MCSH_4_1_transform']))
    except:
        pass

    try:
        temp_list = dataset_setup["MCSH_4_2_r_list"]
        if len(temp_list) > 0:
            for r_list_count in temp_list:
                dataset_name = 'MCSH_4_2_{}'.format(str(r_list_count).replace('.','-'))
                temp_data = np.asarray(data[group_name][dataset_name])
                result_list.append(transform_data(temp_data, dataset_setup['MCSH_4_2_transform']))
    except:
        pass

    try:
        temp_list = dataset_setup["MCSH_4_3_r_list"]
        if len(temp_list) > 0:
            for r_list_count in temp_list:
                dataset_name = 'MCSH_4_3_{}'.format(str(r_list_count).replace('.','-'))
                temp_data = np.asarray(data[group_name][dataset_name])
                result_list.append(transform_data(temp_data, dataset_setup['MCSH_4_3_transform']))
    except:
        pass

    try:
        temp_list = dataset_setup["MCSH_4_4_r_list"]
        if len(temp_list) > 0:
            for r_list_count in temp_list:
                dataset_name = 'MCSH_4_4_{}'.format(str(r_list_count).replace('.','-'))
                temp_data = np.asarray(data[group_name][dataset_name])
                result_list.append(transform_data(temp_data, dataset_setup['MCSH_4_4_transform']))
    except:
        pass
...
def initialize(setup, polynomial_order, fit_intercept):
    os.chdir(setup["model_save_dir"])
    ...
    setup["standard_scaler"] = standard_scaler
    setup["polynomial_order"] = polynomial_order
...
#    functional = setup['functional']
    ...
    initialize(setup, polynomial_order, fit_intercept)
