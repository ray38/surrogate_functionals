...
#    wrapped_n = np.pad(n, 8, mode='wrap')
#    
#    dx = convolve(wrapped_n,Gx)
#    dy = convolve(wrapped_n,Gy)
#    dz = convolve(wrapped_n,Gz)
#
#    temp_gradient = np.add(np.add(dx,dy),dz)
#    result = temp_gradient[8:-8, 8:-8, 8:-8]
#
#    return result
...
def read_integration_stencil(hx, hy, hz, r):
    h = round(((hx + hy + hz) / 3.), 3)
    filename = 'integration_kernel_' + str(h).replace(".","_") + '.p'
    cwd = os.getcwd()
    try:
        os.chdir("/home/ray/Documents/eea/eea/integral_kernels")
        stencil_data = pickle.load(open(filename, "rb"))
        os.chdir(cwd)
    except:
        try:
            os.chdir("/gpfs/pace1/project/chbe-medford/medford-share/users/xlei38/eea/eea/integral_kernels")
            stencil_data = pickle.load(open(filename, "rb"))
            os.chdir(cwd)
        except:
            os.chdir(cwd)
            raise NotImplementedError
    key_name = str(r).replace(".","_")
    pad_key_name = key_name + "pad"
    try:
        stencil = stencil_data[key_name]
        pad = stencil_data[pad_key_name]
        return stencil, pad
    except:
        raise NotImplementedError
    

def read_integration_stencil_file(hx, hy, hz):
    h = round(((hx + hy + hz) / 3.), 3)
    filename = 'integration_kernel_' + str(h).replace(".","_") + '.p'
    cwd = os.getcwd()
    try:
...
#import time
#test_matrix = np.ones((30,30,30))
#test_matrix = np.arange(30*30*30).reshape((30,30,30))
#hx = 0.14
#hy = 0.14
#hz = 0.14
#r = 3.0
#start = time.time()
#result1 = get_integration_fftconv(test_matrix,hx, hy, hz, r, accuracy = 3)
##result2 = get_integration_conv(test_matrix,hx, hy, hz, r, accuracy = 3)
##print result1[0].shape
##print result1[0]- result2[0]
##print np.allclose(result1[0], result2[0])
