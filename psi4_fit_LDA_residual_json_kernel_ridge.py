...
def get_training_data(dataset_name,setup):
    ...
    overall_subsampled_data = subsampling_system_with_PCA(overall_subsampled_data, list_desc = [], cutoff_sig = 0.0005, rate = 0.02,start_trial_component = 9)
...
def fit_model(LDA_result, dens, X_train, residual,kernel):

    kr_model = fit_kernel_ridge(X_train, residual, kernel)
    save_resulting_figure(dens,LDA_result.x,X_train,kr_model,y)

    return kr_model

def fit_kernel_ridge(X,y,kernel):
    kr = KernelRidge(kernel=kernel)
    kr.fit(X,y)

    model_filename = "kernel_ridge.sav"

    pickle.dump(kr, open(model_filename, 'wb'))
...
if __name__ == "__main__":
    ...
    for kernel in ["linear", "rbf"]:
        ...
        model_save_dir = working_dir + "/" + "kernel_ridge_LDA_residual_{}".format(kernel)
