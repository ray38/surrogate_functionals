...
def load_data_each_block(molecule,functional,i,j,k, dataset_setup, molecule_data_directory):
    ...
    return np.asarray(dens), np.asarray(result)
...
def process_each_block(molecule, i,j,k, setup, molecule_data_directory):
    ...
    log(setup["predict_full_log_name"],"\npredicted energy: " + str(y_predict_sum))


    return y_predict_sum
...
def process_one_molecule(molecule, molecule_data_directory, setup):
    ...
    for i,j,k in paramlist:
        ...
        y_predict_sum = process_each_block(molecule, i,j,k, setup, molecule_data_directory)
        system_y_predict_sum += y_predict_sum


    os.chdir(setup["working_dir"])
    log(setup["predict_log_name"],"\n\ndone predicting: " + molecule )
    log(setup["predict_log_name"],"\npredicted energy: " + str(system_y_predict_sum) +  "\ttotal time: " + str(time.time()-system_start_time)+ "\n")

    log(setup["predict_full_log_name"],"\n\ndone predicting: " + molecule )
    log(setup["predict_full_log_name"],"\npredicted energy: " + str(system_y_predict_sum) + "\ttotal time: " + str(time.time()-system_start_time)+ "\n") 
    
    log(setup["predict_error_log_name"], "\n{}\t{}".format(molecule, system_y_predict_sum))
    return system_y_predict_sum
...
def initialize(setup):
    ...
    predict_log_name = "predict_pure_density_log.log"
    predict_full_log_name = "predict_pure_density_full_log.log"
    predict_error_log_name = "predict_pure_density_error_log.log"
...
if __name__ == "__main__":
    ...
    temp_y_predict = process_one_molecule(molecule,molecule_data_directory, setup)
