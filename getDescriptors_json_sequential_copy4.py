...
def process_normal_descriptors(molecule, functional,i,j,k,r_list,asym_list, MCSH_list, MCSH_list2):
    ...
    for r in MCSH_list:
        ...
        temp["MCSH"][dataset_name] = np.asarray(raw_data["MCSH"][dataset_name])
    ...
    with h5py.File(result_filename,'w') as database:
        ...
        for r in MCSH_list:
            original_dataset_name = 'MCSH_1_1_{}'.format(str(r).replace('.','-'))
            dataset_name = 'MCSH_0_1_{}'.format(str(r).replace('.','-'))
            MCSH_grp.create_dataset(dataset_name,data=temp["MCSH"][original_dataset_name])

            original_dataset_name = 'MCSH_2_1_{}'.format(str(r).replace('.','-'))
            dataset_name = 'MCSH_1_1_{}'.format(str(r).replace('.','-'))
            MCSH_grp.create_dataset(dataset_name,data=temp["MCSH"][original_dataset_name])

        for r in MCSH_list2:
            original_dataset_name = 'MCSH_3_1_{}'.format(str(r).replace('.','-'))
            dataset_name = 'MCSH_2_1_{}'.format(str(r).replace('.','-'))
            MCSH_grp.create_dataset(dataset_name,data=temp["MCSH"][original_dataset_name])

            original_dataset_name = 'MCSH_3_2_{}'.format(str(r).replace('.','-'))
...
def process_one_molecule(molecule, functional,h,L,N,r_list,asym_list, MCSH_list, MCSH_list2):
    ...
    for i,j,k in paramlist:
        if i !=0 or j != 0 or k != 0:
            process(molecule, functional,i,j,k,h,N,r_list,asym_list, MCSH_list, MCSH_list2)
...
if __name__ == "__main__":
    ...
    if choice == 'single':
        ...
        MCSH_list = setup['MCSH_list']
        MCSH_list2 = setup["MCSH_list2"]

        #for functional in functionals:
        print "start process molecule"
        process_one_molecule(molecule, functional,h,L,N,r_list,asym_list, MCSH_list, MCSH_list2)
