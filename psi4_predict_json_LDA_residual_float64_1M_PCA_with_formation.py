...
def predict_each_block(setup,dens,X,y):
    ...
    PCA_model = setup["PCA_model"]
    PC_component = setup["PC_component"]

    original_y = detransform_data(y, y_transform)

    X_std = standard_scaler.transform(X)
    X_processed  = transform_pca(PCA_model,X_std,PC_component)

    raw_predict_y = predict_LDA(dens,LDA_model.x) + (NN_model.predict(X_processed*1e6)/1e6)
