...
if __name__ == "__main__":
	...
	for name, group in groups:
		energy_text_dict[name] = "MAE:\n{:6.2f}".format(np.mean(np.abs(group['exc_error'])))
		formation_energy_text_dict[name] = "MAE:\n{:6.2f}".format(np.mean(np.abs(group['formation_exc_error'])))
	...
	for i in range(len(xpos_list)):
		ax1.text(xpos_list[i]-0.25, -11.0, formation_energy_text_list[i],fontsize=25)
	...
	for i in range(len(xpos_list)):
		ax1.text(xpos_list[i]-0.25, 125.0, energy_text_list[i],fontsize=25)
