...
def fit_with_model(density,energy,setup):
    ...
    predict_y = poly_model.predict(density)
...
def predict_poly_residual(n,poly_model,X,NN_model):

    n = np.asarray(n)

    return poly_model.predict(n) + ((NN_model.predict(X*1e6))/1e6)


def save_resulting_figure(n,poly_model,X,NN_model,y,loss,loss_result):

    dens = n
    predict_y = predict_poly_residual(n,poly_model,X,NN_model)
...
def fit_model(poly_model, dens, X_train, residual, y, loss, tol, slowdown_factor, early_stop_trials):

    NN_model,loss_result = fit_with_KerasNN(X_train * 1e6, residual * 1e6, loss, tol, slowdown_factor, early_stop_trials)
    save_resulting_figure(dens,poly_model,X_train,NN_model,y,loss,loss_result)
...
if __name__ == "__main__":
    ...
    model_save_dir = working_dir + "/" + "NN_poly_residual_1M_{}_{}_{}".format(setup["NN_setup"]["number_neuron_per_layer"], setup["NN_setup"]["number_layers"], setup["NN_setup"]["activation"])
    ...
    residual, poly_model = fit_with_model(dens,y,setup)
    ...
    for fit_setup in fit_setup['fit_setup']:
        ...
        tol = fit_setup['tol']
        fit_model(poly_model, dens, X_train, residual, y, loss, tol, slowdown_factor, early_stop_trials*2)
